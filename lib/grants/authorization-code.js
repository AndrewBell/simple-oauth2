...
module.exports = class AuthorizationCode {
  ...
  authorizeURL(params = {}) {
    ...
    const configuredParams = params;
    if (this.config.options.generateState) {
      configuredParams.state = generateState();
    }
  ...
  async getToken(params, httpOptions) {
    ...
    validateState(requestParams.redirect_response_url, requestParams.authorize_url);
    delete requestParams.redirect_response_url;
    delete requestParams.authorize_url;
